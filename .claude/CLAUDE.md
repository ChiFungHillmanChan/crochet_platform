...
- 2026-02-26: signInWithRedirect requires Firebase Hosting at authDomain — use signInWithPopup instead when deploying to S3+CloudFront (no Firebase Hosting)
- 2026-02-26: ADMIN_EMAILS Lambda env var must include ALL admin emails (comma-separated) — also hardcoded in auth-context.tsx for first-time Google sign-in user doc creation
- 2026-02-26: NEVER trust client-sent prices — always verify from Firestore server-side in checkout. Extracted checkout logic to `lambda/admin/checkout.mjs`
- 2026-02-26: Always whitelist fields on update operations (mass assignment protection) — never spread raw request body into Firestore updates
- 2026-02-26: Validate file upload content-type server-side — only allow image MIME types (jpeg, png, webp, gif)
- 2026-02-26: Always escape user-supplied data in HTML email templates with `escapeHtml()` to prevent XSS
- 2026-02-26: Use `FieldValue.increment()` for atomic counter updates (e.g., stock decrement) — never read-then-write which causes race conditions
- 2026-02-26: CORS: return empty string for disallowed origins, not the first allowed origin
- 2026-02-26: Always wrap `JSON.parse()` in try/catch in Lambda handlers — return 400 on invalid JSON
- 2026-02-26: Admin emails on frontend now come from `NEXT_PUBLIC_ADMIN_EMAILS` env var — must be set in `.env.local` and deploy config
